...
import { TrendingUp, Award, CalendarDays, AlertTriangle, GraduationCap, Star, Printer } from "lucide-react";
...
const gradeColor = (g: string) => g === "A" ? "#219EBC" : "#FFB703";
...
export default function Performance() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiClient.get(`/parent/assessment?student_id=${selectedChild.id}`)
    ])
      .then(([gradesRes, attendanceRes, assessmentRes]: any[]) => {
        ...
        setTermData([
          { term: "Current Term", score: gradesRes.average || 0, pos: gradesRes.rank || "—" }
        ]);

        // Trend graph mapping (using subjects and scores)
  ...
  const handlePrint = () => {
    ...
    const url = `${apiBase}/index.php?path=/reports/print&student_id=${selectedChild.id}&token=${encodeURIComponent(token || "")}`;
        ...
        <>
              ...
              <Glass key={t.term} style={{ padding: "18px 20px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "var(--subtext)" }}>Average Score</div>
                  </div>
                  <div style={{ height: 40, width: 1, background: "var(--glass-border)" }} />
                  <div>
                    <div style={{ fontSize: 28, fontWeight: 800, color: "#FFB703" }}>{t.pos}</div>
                    <div style={{ fontSize: 11, color: "var(--subtext)" }}>Class Ranking</div>
                  </div>
                  <div style={{ height: 40, width: 1, background: "var(--glass-border)" }} />
                  <div>
          ...
          <Glass style={{ marginBottom: 18 }}>
            <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--glass-border)", display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontSize: 13.5, fontWeight: 600, color: "var(--heading)" }}>Subject Breakdown</span>
            </div>

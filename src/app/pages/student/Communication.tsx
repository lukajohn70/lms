...
import { MessageSquare, Send, Search, Plus, User, Check } from "lucide-react";
  ...
  const fetchInbox = async (selectId?: number) => {
    try {
      ...
      if (data.threads && data.threads.length > 0) {
        // Keep active selection if possible, otherwise select first
        if (selectId) {
  ...
  return (
    <div>
      <div style={{ marginBottom: 20, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div>
          <div style={{ fontSize: 11, color: "#219EBC", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 4 }}>Inbox</div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--heading)", margin: 0 }}>Communication Portal</h1>
          <p style={{ fontSize: 12.5, color: "var(--subtext)", margin: "4px 0 0" }}>Direct messages with your subject teachers and administrators</p>
        </div>
        <button
      ...
      {loading ? (
        <div style={{ padding: 40, color: "var(--subtext)" }}>Loading inbox threads...</div>
      ) : threads.length === 0 ? (
        <Glass style={{ padding: "60px 40px", textAlign: "center" }}>
          <MessageSquare size={44} style={{ color: "var(--subtext)", opacity: 0.3, marginBottom: 16 }} />
          <h3 style={{ fontSize: 16, fontWeight: 700, color: "var(--heading)", margin: "0 0 8px" }}>Your Inbox is Empty</h3>
          <p style={{ fontSize: 13, color: "var(--subtext)", maxWidth: 360, margin: "0 auto 20px", lineHeight: 1.55 }}>
            You don't have any message threads yet. Click below to compose a message to any of your teachers.
          </p>
          <button
            onClick={handleOpenNewMsg}
              ...
              {filteredThreads.map(t => (
                <div
                  ...
                  style={{
                    ...
                    background: selected === t.id ? "rgba(33,158,188,0.08)" : "transparent",
                    borderLeft: selected === t.id ? "3px solid #219EBC" : "3px solid transparent",
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    {t.unread > 0 && (
                      <div style={{ width: 18, height: 18, borderRadius: "50%", background: "#219EBC", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 700, color: "#fff", flexShrink: 0 }}>{t.unread}</div>
                    )}
                  </div>
                </div>
              ))}
            ...
            {activeThread ? (
              <>
                  ...
                  {activeMsgs.map((m, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: m.self ? "flex-end" : "flex-start" }}>
                      <div style={{
                        ...
                        background: m.self ? "linear-gradient(135deg, #219EBC, #023047)" : "var(--muted)",
                ...
                <form onSubmit={handleSend} style={{ padding: "14px 20px", borderTop: "1px solid var(--glass-border)", display: "flex", gap: 10 }}>
                  ...
                  />
                  <button type="submit" style={{ padding: "10px 18px", borderRadius: 10, background: "linear-gradient(135deg, #219EBC, #023047)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, color: "#fff", fontSize: 13, fontWeight: 600 }}>
                    <Send size={14} /> Send
                  </button>
                </form>
              </>
          ...
          <Glass style={{ width: 420, padding: 24 }}>
            ...
            <form onSubmit={handleSendNew} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div>
                ...
                {teachers.length === 0 ? (
                  <div style={{ fontSize: 12, color: "var(--subtext)" }}>No teachers available.</div>
                ) : (
                  <select
              ...
              <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
                ...
                <button type="submit" disabled={sendingNew || !selectedTeacherId} style={{ flex: 1, padding: 10, borderRadius: 8, background: "#219EBC", border: "none", color: "#fff", cursor: "pointer", fontWeight: 600, fontSize: 13 }}>
